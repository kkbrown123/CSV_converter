...
def write_xml(folder, filename, bbox_list):
    ...
    for entry in bbox_list:
        ...
        if len(e_ymin.split(' ')) > 1:
            for i in range(len(e_ymin.split(' '))):
                ...
                SubElement(bbox, 'xmin').text = e_xmin.split(' ')[i]
                SubElement(bbox, 'ymin').text = e_ymin.split(' ')[i]
                SubElement(bbox, 'xmax').text = e_xmax.split(' ')[i]
                SubElement(bbox, 'ymax').text = e_ymax.split(' ')[i]
        else:
            obj = SubElement(root, 'object')
            SubElement(obj, 'name').text = e_class_name
            SubElement(obj, 'pose').text = 'Unspecified'
